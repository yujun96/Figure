<template>
  <scroll class="listview" :data="data" ref="listview" :listenScroll="listenScroll" @scroll='scroll' :probeType='probeType'>
    ...
    <div class="list-shortcut" @touchstart='onShortcutTouchStart' @touchmove.stop.prevent='onShortcutTouchMove'>
      <ul>
        <li v-for="(item,index) in shorcutList" :key="index" :data-index="index" :class="{item:true,current:currentIndex===index}">
          {{item}}
        </li>
      </ul>
    </div>
...
export default {
  ...
  methods: {
    ...
    onShortcutTouchMove(e) {
      ...
      let delta =
        Math.round((this.touch.y2 - this.touch.y1) / ANCHOR_INDEX) | 0;
