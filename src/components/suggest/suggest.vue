  ...
  <scroll
    ...
    <ul class="suggest-list">
      <li  class="suggest-item" v-for="item in result" :key="item.songid"  @click="selectItem(item)">
        <div class="icon">
          <i :class="getInconCls(item)"></i>
        </div>
        <div class="name">
          <p class="text" v-html="getDisplayName(item)"></p>
        </div>
      </li>
        <loading v-show="hasMore" tittle=""></loading>
    </ul>
...
export default {
    ...
    methods:{
      ...
      selectItem(item){
          if(item.type===TYPE_SINGER){
             const singer = new Singer(item.zhida.singermid,item.zhida.singername)
              this.$router.push({
     ...
     _genResult(data){
         ...
         if(data.zhida && data.zhida.singerid){
           ret.push({...data,...{type:TYPE_SINGER}})
         }
        if(data.song){
